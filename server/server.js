...
// Haal de database op. We zorgen dat als 'darters' ontbreekt bij een toernooi, we een lege lijst maken.
function readDB() {
...
// NIEUW: Haal alleen de namen van de toernooien op voor het startscherm
app.get('/api/tournaments', (req, res) => {
    const db = readDB();
    res.json(db.tournaments.map(t => t.name));
});

// AANGEPAST: Haal matches op voor EEN SPECIFIEK toernooi
app.get('/api/matches', async (req, res) => {
    ...
    try {
        // Haal alleen data op van dit ene toernooi
    ...
    // Pak nu alleen de darters van DIT toernooi
    ...
    eigenWedstrijden.forEach(match => {
        ...
        if (match.status === "gespeeld") {
            let onzeSpeler = tournament.darters.find(d => 
    ...
    try {
        ...
        if (matchesList.length > 0) {
            const matchesMetToernooi = matchesList.map(m => {
                ...
                // NIEUW: Haal de schrijver (Marker) op!
                ...
                if (Array.isArray(markerData) || typeof markerData === 'number') {
                    ...
                    markerNaam = markerData; // Bijv: "Voluntary marker"
                }
                ...
                return {
                    ...
                    player1: getSpelerNaam(m.d1 || m.p1),
                    player2: getSpelerNaam(m.d2 || m.p2),
                    marker: markerNaam, // Sla de marker op
                    score1: m.s1 !== null && m.s1 !== undefined ? m.s1 : "",
                    score2: m.s2 !== null && m.s2 !== undefined ? m.s2 : "",
                    board: m.bn || m.b || m.board || m.bd || "?",
                    time: m.tm || m.t || m.time || m.st || "Later",
    ...
    let eigenWedstrijden = rawMatches.filter(match => {
        // We filteren nu of onze darter speler 1 is, speler 2 is, OF de marker is!
    ...
    eigenWedstrijden.forEach(match => {
        ...
        // Is onze darter de speler of de marker?
        ...
        // NIEUW: Als hij ALLEEN marker is en de wedstrijd is gespeeld... negeer deze wedstrijd dan compleet!
        if (!isSpeler && isMarker && match.status === "gespeeld") {
            return; 
        }

        // Voeg rol toe voor het dashboard
        match.rol = (!isSpeler && isMarker) ? "marker" : "speler";

        // Resultaatkleuren bepalen (alleen als hij zelf speelt)
    ...
    // Mogelijke wedstrijden berekenen (Alleen als hij als SPELER speelt en wint of nog moet spelen)
    eigenWedstrijden.forEach(match => {
